pipeline {
    ...
    environment {
        NAME_CONTAINER = "roadwaymanager-microservice-psm"
        NAME_IMAGE = "roadwaymanager-image-psm:1"
        ID_CONTAINER = null 
        PORT_CONTAINER = "9008:9008"
    }
    ...
    stages {
        ...
        stage('Git Checkout Repositorio') {
            steps {
                git branch: 'develop',
                url: 'https://github.com/packsendme/packsendme-roadwaymanager-server.git'
            }
        }
        stage('Java Build') {
          steps {
